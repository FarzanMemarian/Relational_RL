...
class hDQN:
    ...
    def Q_cntr(self, agent_env_state, goal, target):
        if target:
            try:
                ...
                goal = torch.unsqueeze(torch.tensor([goal], dtype=torch.float32, device=self.device), dim=0)
                Q_policys = self.target_cntr_net(agent_env_state, goal) 
        else:
            try:
    ...
    def _update_cntr(self):
        ...
        next_state_Vs = torch.zeros(sample_size, device=self.device)
        all_cntr_done = False
        try:
        ...
        reward_batch = torch.tensor([exp.int_reward for exp in exps], dtype=torch.float32, device=self.device).detach()
        action_batch = torch.unsqueeze(torch.tensor([exp.action_idx for exp in exps], device=self.device), 1).detach()
    ...
    def _update_meta(self):
        ...
        reward_batch = torch.tensor([exp.reward for exp in exps], dtype=torch.float32, device=self.device)
        ...
        Q_targets = torch.zeros((sample_size, 1), device=self.device)
        ...
        for i, exp in enumerate(exps):
            if not exp.terminal:
                ...
                next_goals = torch.cat([torch.unsqueeze(torch.tensor([next_goal], dtype=torch.float32, device=self.device), 0)
